...
# QML - appVers property
sed -i "s/readonly property string appVers: \"[0-9.]*\"/readonly property string appVers: \"${NEW_VER}\"/" "$SCRIPT_DIR/plasma6_4_main.qml"

echo "Done. Files updated:"
grep -rn "3\.\|APP_VER\|appVers\|PluginInfo-Version" "$SCRIPT_DIR" \
    --include="*.sh" --include="*.qml" --include="*.json" --include="*.desktop" \
    | grep -E "(APP_VER=|appVers:|PluginInfo-Version=|${NEW_VER})" \
    | grep -v ".git"
