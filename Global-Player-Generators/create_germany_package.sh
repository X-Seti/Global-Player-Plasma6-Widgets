#!/usr/bin/env bash
# Global Player Germany Package Generator
set -euo pipefail

PACKAGE_NAME="Global-Player-Germany"
PLASMOID_ID="org.mooheda.globalplayer.germany"

echo "[+] Creating Global Player Germany package for Plasma 6..."
...
echo "[+] Creating metadata.json for Germany version..."
cat > "${PLASMOID_ID}/metadata.json" << 'EOF'
{
    "KPlugin": {
        ...
        "Category": "Multimedia",
        "Description": "Höre deutsche Radiosender (RTL Radio, 1LIVE, Bayern 3, etc.)",
        "Icon": "audio-headphones",
        "Id": "org.mooheda.globalplayer.germany",
        "Name": "Global Player Deutschland",
        "Version": "3.2",
        "Website": "",
        "BugReportUrl": ""
    },
...
echo "[+] Creating main.qml (Germany version)..."
...
echo "[+] Creating MPV player (Germany version)..."
...
class MPVPlayer:
    def __init__(self, socket_path=None):
        self.proc = None
        self.sock_path = socket_path or os.path.join(tempfile.gettempdir(), "gpd-germany-mpv.sock")
        self.state = "Stopped"
...
cat > "gpd-germany.service" << 'EOF'
[Unit]
Description=Global Player Deutschland Daemon v3.2

[Service]
Type=simple
ExecStart=/usr/bin/env python3 %h/globalplayer-daemon-germany/gpd.py
Restart=on-failure
...
echo ""
echo "🇩🇪 Global Player Deutschland Paket erfolgreich erstellt!"
echo ""
echo "📁 Paket: ${PACKAGE_NAME}/"
echo ""
echo "📻 Deutsche Radiosender:"
echo "   • Öffentlich-rechtlich: 1LIVE, WDR 2, Bayern 3, SWR3, NDR 2"
echo "   • Jugendwellen: MDR Jump, Radio Fritz, DLF Nova"
echo "   • Private: Antenne Bayern, BigFM, Energy Berlin"
echo "   • Spezial: Deutschlandfunk, Klassik Radio, Radio Bob"
echo ""
echo "✨ Features:"
echo "   • 🇩🇪 Deutsche Flagge und Gold-Akzente"
echo "   • 🔔 Deutsche Lokalisierung"
echo "   • 🎨 Album-Artwork Support"
echo "   • 📝 Deutsches Logging"
echo ""
echo "🚀 Installation:"
